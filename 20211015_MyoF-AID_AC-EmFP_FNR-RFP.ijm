...
run("Z Project...");
	...
	run("Split Channels");
	//casette 1 begin
	selectWindow("C1-MAX_"+imageTitle);//select channel 1
	...
	run("Green");//alter the LUT to match whichever color you want for C1
	//casette 1 end
	
	//casette 2 begin
	selectWindow("C2-MAX_"+imageTitle);//select channel 2
	C2=getTitle();//saves channel 2 name as a variable
	if (getBoolean("reset to min/max of image pixels? or specified values in the script for comparison?", "fit to image", "specific values")) {
	resetMinAndMax ;}
	else {setMinAndMax(0, 10000);//manually set these values for  your data
	}
	run("Magenta");//alter the LUT to match your C2
	//casette 2 end
