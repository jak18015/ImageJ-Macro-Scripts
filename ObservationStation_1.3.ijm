...
// returns the index of the chosen image set for correctly applying the recorded data to it		
		...
		Dialog.addString("List of descriptors", "# of parasites in vacuole, division state, actin dynamics, D1, D2, D3", 100);
		Dialog.show();
		...
		for (j=0; j < DvArray.length; j++) {
			...
			if (channels > 1) {
				...
				if (channels + slices = 1) {
					setLocation(screenWidth/2.8, screenHeight/3);
					selectWindow(title);
						run("Start Animation");
				}
			}
			...
			close("*" + DvArray[j] + "*");
			Table.save(TableDir + TableName + ".csv");
		}
		showMessage("Table saved to: " + TableDir + TableName + ".csv");
	}
}
...
function CenterAndZoom() {
	selectWindow("C1-" + title);
		setLocation(screenWidth/50, screenHeight/2.5);
		run("In [+]");
		run("In [+]");
		run("Magenta");
	selectWindow("C2-" + title);
		setLocation(screenWidth/3.1, screenHeight/2.5);
		run("In [+]");
		run("In [+]");
		run("Green");
	selectWindow(title);
		setLocation(screenWidth/1.6, screenHeight/2.5);
		run("In [+]");
		run("In [+]");
		Stack.setChannel(1);
		run("Magenta");
