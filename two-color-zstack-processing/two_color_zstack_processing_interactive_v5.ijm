...
// -----------------------------------------
convolution_choice_array = newArray("R3D", "D3D", "Both");
Dialog.create("Do you want to process raw or deconvolved images or both?");
	Dialog.addMessage("_____________________________________________________________");
	Dialog.addChoice("Convolution", convolution_choice_array);
Dialog.show();
	convolution_choice = Dialog.getChoice();
	if (convolution_choice == "R3D") {
...
Table.create("processing_record.csv");
Table.setColumn("Image", list);
Table.setLocationAndSize(0, 0, 600, 1000);
...
// run("Arrange Channels...");
	if (getBoolean("Do you want to pick images one by one, or start at an image and loop through the end of set?", "Single Image", "Loop") == true) {
		...
		Dialog.create("Choose a specific image");
			...
			img = Dialog.getChoice();
			print("Choice: " + img);
			Array.show("Results (indexes)", list);
			for (i=0; i < nResults; i++) {
				if (getResultString("Value", i) == img) {
					index = getResult("Index", i);
					}
			}
			newlist = Array.slice(list,index,list.length);
			run("Close");
			print("-------------------------------");
			print("Subset of files for processing");
...
Table.save(output + "processing_record.csv");
showMessage("Processing Done!");
...
run("In [+]");
	...
	selectWindow(title);
	projection_type_array = newArray("[Max Intensity]", "[Min Intensity]", "[Sum Slices]", "[Standard Deviation]", "[Median]");
	Dialog.createNonBlocking("Set Z-stack measurements");
	Dialog.setLocation(1,1);
	Dialog.addMessage("Unlike the regular Z-projection menu, you can view your image simultaneously with this window", 12, "Red");
	Dialog.addString("Start slice:", "1", 15);
	Dialog.addString("Stop slice:", slices);
	Dialog.addChoice("Projection type:", projection_type_array);
	Dialog.show();
	start_slice = Dialog.getString();
	stop_slice = Dialog.getString();
	projection_type = Dialog.getChoice();
	
		for (j=0; j < list.length; j++) {
			if (Table.getString("Image", j) == title) {
				Table.set("Start Slice", j, start_slice);
				Table.set("Stop Slice", j, stop_slice);
				Table.set("Projection Type", j, projection_type);
				Table.update;
				}
		}
	run("Z Project...", "start=" + start_slice + " stop=" + stop_slice + " projection=" + projection_type);
