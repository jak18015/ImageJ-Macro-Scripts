...
for (image = 0; image < list.length; image++) {
	...
	for (g=0; g < actinCount; g++) {
		Dialog.createNonBlocking("Actin segment " + g+1 + "");
		Dialog.addNumber("Starting Frame: ", 1);
		Dialog.addNumber("Ending Frame: ", 50);
		Dialog.show();
		F0 = Dialog.getNumber();
		F1 = Dialog.getNumber();
		selectWindow(title);
		run("Make Substack...", "slices=" + F0 + "-" + F1 + "");
	...
	// set up arrays for table
		...
		// actin number array
		actinArray = newArray();
		// Frame begin and end arrays
		f0Array = newArray();
		f1Array = newArray();
		
		selectWindow(title);
		
		// propogate other arrays to the same size as the pixel array
		for (i=0; i < pixelArray.length; i++) {
			micronArray[i] = pixelArray[i]*micronPerPixels;
			titleArray[i] = title;
			dirArray[i] = getDirectory("image");
			actinArray[i] = g+1;
			f0Array[i] = F0;
			f1Array[i] = F1;
		}
		...
		Table.setColumn("F0", f0Array);
		Table.setColumn("F1", f1Array);
